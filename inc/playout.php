...
class Playout
{
	...
	/**
	 * Pause the specified unit
	 *
	 * @access    public
	 * @param     $unit (optional)
	 * @return    boolean
	 */
	function pause($unit = 0) {
		$this->command('pause', array(), $errors, $unit);
		return $errors;
	}
...
class MvcpClient extends Client
{
	...
	function status() {
		...
		list($unit, $mode, $currentClip, $currentFrame, $speed, $fps, $in, $out,
			 ...
			 $seekable, $playlistGenNumber, $clipIndex) = explode(' ', $status);
